...
dev_docs.py inventory --dir <目标项目> [--out <子目录名=dev-docs>] [--exclude 额外排除]
                      [--include 排除白名单] [--project-type auto|catkin|ros2|java|node|generic]
                      # 文件全集 + 符号/端点 + 哈希基线；解析 package.json/tsconfig/pyproject/
                      # requirements.txt（entries/external_deps/路径别名，v1.7.0）；monorepo 工作区
                      # 分组（modules[].group + module_groups）；JS/TS HTTP 端点与 .vue 符号抽取；
                      # --include 解除同名默认排除；解析失败文件汇总在 scan_errors（--strict ERROR）
