...
4. 依赖分析（v1.5 已实现；v1.5.8 扩展）：python import 与 C++ `#include` 自动映射为 `modules[].deps`（包内互引）；python 增加**包名探测**——目录含 `__init__.py` 时按末段目录名匹配 import（`src/app/__init__.py` ↔ `import app.core`）；JS/TS 相对导入（`./`/`../`）按文件解析、`@/` 按 `src/` 根别名约定解析；CMake `find_package` 与 `package.xml` 的 depend 系标签 → `modules[].external_deps`；ROS 调用形态（`advertise`/`subscribe`/`advertiseService`/`serviceClient`/`ros::init` 与 rospy 对应形态）→ `inventory.interfaces` 的 `TOP-/SVC-/NDE-` 条目。剩余细节由 AI 在 architecture/模块四问里补（evidence: 事实——文件路径）。
